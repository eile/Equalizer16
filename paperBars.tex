...
%DIF DEL submission1.tex   Sun Aug 12 12:15:12 2018
%DIF ADD paperIEEE.tex     Thu Aug 16 11:33:24 2018
...
\author{Stefan~Eilemann\thanks{email: eilemann@gmail.com},
        ...
        and~Renato~Pajarola\DIFdelbegin %DIFDELCMD < \thanks{email:pajarola@acm.org}%%%
\DIFdelend \DIFaddbegin \thanks{email: pajarola@ifi.uzh.ch}\DIFaddend ,~\IEEEmembership{Senior Member,~IEEE}%
...
%DIF < pajarola@acm.org}}
%DIF > pajarola@ifi.uzh.ch}}
...
%DIF <  This section introduces new decomposition modes, runtime adaptations and other
%DIF <  optimizations to increase the rendering performance for a wide variety of
%DIF <  algorithms and use cases.
...
%DIF <  Furthermore, subpixel compounds interact with idle image refinements, e.g., they
%DIF <  can accelerate idle anti-aliasing of a scene when the camera and scene are not
%DIF <  changed.
...
unfavorably assigned under such conditions. \DIFaddbegin \DIFadd{Sort-first performs better for
volume rendering since it is easier to load balance than in our polygonal
renderer due to the predictable per-fragment cost. The polygonal renderer
performs better in sort-last rendering, since sort-first is harder to load
balance and observes inefficencies when geometry is intersected by a load split
and has to be rendered by both resources.
}\DIFaddend 
...
This figure gives an impression of how individual modes perform on
heterogeneous systems. In this case the tree equalizer performs best
(\fig{fEqualizers} (left)), as it allows us to a priori define how much {\em
usage} it should make of individual nodes, i.e., bias the allocation of
rendering time in accordance with the (simulated) compute resources.
\fig{fEqualizers} (right), on the other hand, illustrates how the tested
...
virtualized enviroments. For this scenario \DIFdelbegin \DIFdel{tile
equalizer }\DIFdelend \DIFaddbegin \DIFadd{\textsf{tile\_equalizer} }\DIFaddend seems best
suited, as it performs load balancing implicitly and does not assume coherence
of available resources between frames. The simpler tree equalizer also
outperforms the \DIFdelbegin \DIFdel{load equalizer }\DIFdelend \DIFaddbegin \DIFadd{\textsf{load\_equalizer} }\DIFaddend in this experiment.
