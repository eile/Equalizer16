...
being implicitly, i.e., inherently load-balanced, as long as there is an insignificant overhead for the render task setup. This mode is transparent to \textsf{Equalizer} applications.
...
\subsubsection{Implicit Load Balancing}

The package equalizer allows to adjusts the implicit load balancing that results from rendering clients pulling work packages, i.e., tiles and chunks, from a central queue on the server. It is implemented as \textsf{tile\_equalizer} and \textsf{chunk\_equalizer}, respectively, and changes the affinity of work packages to individual nodes, based on an affinity model specified to the equalizer. In~\cite{SPEP:16}, we explore this approach in more detail.
...
optimizations in \textsf{Collage} affect complex operations in realistic
scenarios.
...
to continuously modify the rendering load the system has to cope with. In both cases
the scene comprised of two rows of instantiated, identical models with 30 models
in total. Rendering was performed at an output resolution of $2560\times1440$. The camera is
...
all rendered frames for each run (\fig{fCompounds}), backing up along the alley of models.
% We provide the results of the load balancer providing the better result for each application and compound.\FIXME{what does this last sentence mean?}
...
counterintuitive result seems to again confirm the idea that simple algorithms often outperform theoretically better, but more complex implementations. On the other hand, \textsf{tile\_equalizer} often outperforms \textsf{tree\_equalizer}. This suggests that the underlying implicit load balancing can be superior to the the explicit methods of \textsf{load\_equalizer} and \textsf{tree\_equalizer} in high load situations, where the additional overhead of tile generation and distribution is more justified.

\FIXME{oscillation, pixel, clearer}
% \FIXME{I'm not sure I read Fig.10 the same way. Left it seems that tile\_equalizer is the best for polygonal rendering, it's constantly faster than the tree\_equalizer mentioned in the paragraph above.}
...
\begin{figure}[ht]\center
  ...
  \caption{\label{fBenchmarkEVolve} Flower alley with 30 volume models of $1024^3$ unsigned bytes (1\,GB) each.}
\end{figure}

\begin{figure}[ht]\center
  \includegraphics[width=.9\columnwidth]{images/benchmark_eqPly2}
  \caption{\label{fBenchmarkEqPly} David alley with 30 statues of 56\,M triangles (988\,MB) each.}
\end{figure}
...
dual processor Intel Xeon X5690 CPUs (3.47\,GHz), 24\,GB main memory per node, 10\,GBit/s
