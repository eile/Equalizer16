...
\IEEEtitleabstractindextext{%
  \begin{abstract}
    ...
    applications. Using the \textsf{Equalizer} parallel rendering framework, we
    show how a wide set of generic algorithms can be integrated in the framework
    to help application scalability and development in many different domains,
    highlighting how concrete applications benefit from the diverse aspects and
    use cases of \textsf{Equalizer}. We present novel parallel rendering
    algorithms, powerful abstractions for large visualization setups and virtual
    reality, as well as new experimental results for parallel rendering and data
    distribution.
  \end{abstract}
...
The Equalizer parallel rendering framework, first presented in~\cite{EMP:09},
has demonstrated its general versatility and the usefulness of its
minimally-intrusive design in a variety of applications and projects. In
...
release of Equalizer, many features and functionalities have not been published
yet and are presented here. In this report, we thus present an updated
...
usability features to build complex applications, an overview of the underlying
\textsf{Collage} network library and a summary of the main
\textsf{Equalizer}-based applications. A result section presents new
experiments, followed by the discussion and conclusion.
...
\textsf{Equalizer}~\cite{Eilemann:13}. We assume these two publications and
...
The core task of developing an Equalizer application is to separate the
...
\begin{compactdesc}
 ...
 \item[Head Transformation] A transformation matrix positioning the frustum.
...
An Equalizer configuration determines how the application is executed in
...
Our execution model is fully asynchronuous, and only introduces synchronization
...
In the following we present new decomposition modes and motivate their use
...
mostly useful for non-interactive rendering, e.g., for movie generation. It is
...
DPlex is configured by restricting the source compounds to a \textit{phase}
...
sort-last rendering, respectively. They are implemented via the \textsf{tile\_equalizer} and \textsf{chunk\_equalizer} compounds, which decompose the scene into a predefined
set of fixed-size image tiles or database ranges, respectively.
The resulting tasks, or work packages,
...
As shown in~\cite{SPEP:16} and the results, these modes can provide better
performance due to being implicitly, i.e., inherently load-balanced, as long as
there is an insignificant overhead for the render task setup. This mode is
transparent to \textsf{Equalizer} applications.

Tile and chunk compounds are configured through output and input queues, which
...
Pixel compounds are configured similarly to sort-first compounds, except that
...
Subpixel compounds are configured similarly to pixel compounds, but instead of
...
\textsf{load\_equalizer} and a \textsf{tree\_equalizer}. Both implementations
modify the \textsf{viewport} or \textsf{range} of their direct child compounds.
The result section provides some evidence on the strengths and weaknesses of
both algorithms.
...
Cross-segment load balancing (CSLB, \fig{fig:vieweq}) optimizes resource
...
CSLB is unique in that it supports load balancing for planar and non-planar
visualization systems, as well as a flexible allocation of rendering resources
to individual output channels. In \cite{EEP:11}, we provide a detailed
description and evaluation of our algorithm and show that it significantly
improve performance over a static allocation.
...
rendered by a single resource. We use an application-provided ROI to optimize
the \textsf{load\_equalizer} as well as image compositing in our framework. The
...
most applications and large data scenarios. We implement three threading modes:
...
Virtual Reality is an important field for parallel rendering, which requires
...
Head tracking is fundamental to support immersive installations. We support
multiple, independent tracked views through the observer abstraction
(\sref{sec:observer}). Built-in VRPN support enables direct,
...
%------------------------------------------------------------------------------
\subsection{Physical and Logical Visualization Setup}\label{sView}
...
display wall through \textsf{Collage} objects. The control host allows full
...
\textsf{Collage} is a separate, standalone library heavily influenced by the
...
RTT Deltagen (\fig{fLayout}, now Dassault 3D Excite) is a commercial
...
RTT Scale uses a master-slave execution mode, where a single running Deltagen
...
visualisation of neuronal simulations based on cable models (\fig{FIG_teaser}
(a)). It is based on OpenSceneGraph for data management and Equalizer for
...
% Time to render a 4k, 32-step MSAA image of the 1\,mm David model and Livre using 1 to 10 nodes with all modes (2D, 2D LB, DB, DPlex (over AA steps), tiles, chunks, pixel, subpixel) (+DFR for Livre).
...
nodes are added to the task, due to unfavorable assignment of a tile with a high
work load on odd node counts. Static sort-last volume rendering has a similar
oscillation behaviour, as ranges of scene geometry tend to also get unfavorably
assigned under such conditions. It seems plausible that this is also due to overheads associated with these two rendering scenarios. That is, volume rendering typically benefits less form sort-last decompositions, because of higher costs for compositing. Similarly, polygonal rendering often benefits less from sort-first decomposition, as compositing costs are typically much lower, while culling of geometry can create an additional overhead.
...
for shared rendering nodes in virtualized enviroments. For this scenario \textsf{tile\_equalizer} seems best suited, as it performs load balancing implicitly and does
not assume coherence of available resources between frames. The simpler tree
equalizer also outperforms the \textsf{load\_equalizer} in this experiment.

\begin{figure*}[ht]%
\includegraphics[height=.17\textwidth]{images/benchmark_eVolve}\hfil
\includegraphics[height=.17\textwidth]{images/benchmark_eqPly2}\hfil
\includegraphics[height=.17\textwidth]{images/spikes512}\hspace{.2mm}\includegraphics[height=.17\textwidth]{images/beechnut}
  \caption{\label{fBenchmarks}Benchmark data: Flower alley with 30 volume models
  of $1024^3$ unsigned bytes (1\,GB) each (left); David alley with 30 statues of
  56\,M triangles (988\,MB) each (middle); Volumes used for object distribution
  benchmarks (right): spike frequencies of a three million neuron electrical
  simulation at $512\times437\times240$ unsigned byte (51\,MB) and an MicroCT scan of a
  beechnut at $1024\times1024\times1546$ unsigned short (3\,GB) resolution}
\end{figure*}
...
found in the
\htmladdnormallinkfoot{implementation}{https://github.com/Eyescale/Equalizer/tree/paper2018/tools/eqObjectBench}.
...
mapping is a slave-triggered ``pull'' operation. During commit, the buffers
...
synchronization. In contrast, object mapping needs to wait for each slave node
...
total of $1023$ objects are distributed and synchronized. \fig{fDavidDist} shows
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{images/david_steiner_picture2}}]{David Steiner}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{images/renato}}]{Prof. Dr. Renato Pajarola}
