...
optimizations~\cite{EBAHMP:12}; as well as work queueing~\cite{SPEP:16}. \FIXME{Why ; instead of , ?}
...
coherency. \FIXME{Shouldn't that be frame-to-frame {\em coherence}?} Equalizer implements two different algorithms, a
...
\item[Unbuffered] The object is versioned and unbuffered. No data is stored, and
  no previous versions can be mapped. \FIXME{Redundant.} No old versions can be mapped and no
  additional data is stored.
\end{compactdesc}

Serialization is facilitated using output or input streams, which abstract the
data transmission and are used like an \textsf{std::stream}. The data streams
...
\item Newly mapped slave instances are mapped to the oldest available
  version by default \FIXME{Is this correct?}, or to the version specified when calling
  \textsf{mapObject}.
\end{compactitem}
...
registered master objects to all connected
nodes, while the corresponding node is idle. These nodes simply enter the received data to their cache. Preloading
...
\textsf{Equalizer} and other applicatons. A barrier is a distributed barrier
primitive used for software swap barriers in \textsf{Equalizer}
(\sref{sec:swap}) \FIXME{Using the word barrier three times in one short sentence}. Its implementation follows a simple master-slave approach,
which has shown to by sufficient for this use case. The used distributed data structures are single producer,
multiple consumer FIFO queues. To hide network latencies, consumers prefetch stored
items into a local queue. Queues are used for tile and chunk compounds
(\sref{sec:tile}).
...
simplify the integration of third-party libraries like OpenGL \FIXME{But if it's based on Equalizer then OpenGL should already be part of the backend?}, OpenSceneGraph,
...
oscillation behaviour, as ranges of scene geometry tend to also get unfavorably assigned under such conditions.
...
application's rendering algorithms is also favoring work packages, since they have
a near-zero static overhead per rendering pass.

Finally, we also provide scalability results for pixel compounds.
While naturally load-balanced, pixel compounds only scale fill rate and not
geometry processing. Consequently, pixel compounds provide better performance
for volume rendering, and a predictable scaling behaviour for both.
...
for shared rendering nodes in virtualized enviroments. For this scenario tile equalizer seems best suited,
...
\begin{figure*}[ht]\center
    \includegraphics[height=.17\textwidth]{images/benchmark_eVolve}
    \includegraphics[height=.17\textwidth]{images/benchmark_eqPly2}
    \includegraphics[height=.17\textwidth]{images/spikes512}
    \includegraphics[height=.17\textwidth]{images/beechnut}
...
files \FIXME{What are these files? Fig. 7 only speaks of "generic" binary data...}, precalculating the speed and compression ratio of the various engines.
