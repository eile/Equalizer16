...
\textsf{Equalizer} project name. \FIXME{modified paragraph is ok; but we
still must make sure that the font differs between the Equalizer project and the
Equalizer components consistently throughout the text}
...
% Was meinst du damit konkret? Die Implementierung verwendet einen impliziten Octtree (dh kein “baum” ist im Speicher, die Koordinaten werden anhand der Tiefe & position implizit berechnet), asynchrones laden von der DataSource, asychronen Upload auf die GPU. Die “source” Datenstruktur kennt Livre nicht, das hängt von der Implementierung der Datasource ab (zZ UVF, RAW (one octree node), Fivox (3D events-to-volume), Cubist (mesh-to-volume)).

% A VisibleNodesVisitor traverses the Octtree to select the nodes based on the given screen space error (~max pixel/voxel) and their availability. How the nodes are selected, in particular when between different neighbour LOD levels is beyond my knowledge. Another (?) visitor is used to select desired nodes, which are submitted to the data loader threads, which use the data source to load them, then upload them (async) and add them to the available set.
