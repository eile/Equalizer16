...
\FIXME{Highlight integration of many features into a general framework vs one-off
research implementation of one algorithm. Also point out the various and very
diverse examples in Section 7, highlighting the variety of application and
implementation
...
build complex applications, main novel features of the underlying
\textsf{Collage}
network library and a quick overview of the main \textsf{Equalizer}-based
applications. A
...
In 2009 we presented the initial \textsf{Equalizer} parallel rendering
framework~\cite{EMP:09}, which introduced the architecture of a generic parallel
rendering frameworks and summarized the work in parallel rendering. Since then,
an extensive Programming and User Guide provides in-depth documentation on using
and programming \textsf{Equalizer}~\cite{Eilemann:13}. In the following related
work we assume these two publications and their references as a starting point,
and focus on the new work published since 2009.
...
present the newly added decomposition modes and motivate their use case.
\fig{fig:compounds}
...
threads asynchronously, as outlined below in \sref{sec:threading}. The output
frame
...
installations. \textsf{Equalizer} does support multiple, independent tracked
views
...
be computed. \textsf{Equalizer} supports this through the optional
...
\textsf{Equalizer} applications to define view-specific data for rendering,
e.g., a
...
and robustness, \textsf{Equalizer} will flush the rendering pipeline whenever a
runtime
...
schedulers. This feature is transparent to \textsf{Equalizer} application
developers.
...
compared to \textsf{Equalizer}. In \textsf{Equalizer}, each GPU rendering
thread is independently
...
\textsf{Sequel} is a simplification layer for \textsf{Equalizer}. It is based
on the realization
that while fully expressive, the verbatim abstraction layer of nodes, pipes,
windows and channels in \textsf{Equalizer} requires significant learning to
fully
...
support for scalable rendering modes, many of which presented for the first
time. We present new runtime adaptations for better load balance and
