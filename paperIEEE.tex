...
which has shown to by sufficient for this use case. Queues are single producer,
multiple consumer FIFOs. To hide network latencies, consumers prefetch queue
items into a local queue. They are used for tile and chunk compounds
(\sref{sec:tile}).
...
application's rendering algorithms is also favoring work packages, since it has
a near-zero static overhead per rendering pass.

Last, but not least, we also provide scalability results for pixel compounds.
