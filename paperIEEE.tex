...
retransmit packets from nack datagrams, and advance their
...
buffers or finishes its write queue. Nack datagrams may contain multiple ranges
of missed datagrams, as UDP implementations often drop multiple packets at
once.
...
\begin{figure*}[ht]\center
  \includegraphics[width=\textwidth]{images/daviddist}
...
\begin{figure*}[ht]\center
  \includegraphics[width=\textwidth]{images/ifdist}
...
peer-to-peer connection benchmark. This needs further investigations, but we
suspect the abstraction of a byte stream connection chosen by \textsf{Collage}
for historical reasons is not will suited for remote DMA semantics, that is,
...
process per node is used (\fig{fIFDist}, middle: 30\% slower on ethernet, flat
on InfiniBand). The increased transmission time with multiple clients is caused
by a higher probability of packet loss, which increases significantly when using
more than one process per node, as shown in \fig{fIFDist}, right. Infiniband
outperforms ethernet slightly, but is largely limited by the RSP implementation
throughput of preparing and queueing the datagrams to and from the protocol
thread, which we observed in a profiling analysis.
