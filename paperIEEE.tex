...
The package equalizer allows to adjusts the implicit load balancing that results from rendering clients pulling work packages, i.e., tiles and chunks, from a central queue on the server. It is implemented as \textsf{tile\_equalizer} and \textsf{chunk\_equalizer}, respectively, and changes the assignment of work packages to individual nodes, based on an affinity model specified to the equalizer. In~\cite{SPEP:16}, we explore this approach in more detail.
