...
assigned under such conditions.
...
for shared rendering nodes in virtualized enviroments. For this scenario tile
equalizer seems best suited, as it performs load balancing implicitly and does
not assume coherence of available resources between frames. The simpler tree
equalizer also outperforms the load equalizer in this experiment.
...
mapping is a slave-triggered ``pull'' operation. Furthermore, the buffers only
have to be serialized and compressed once, and then can be sent directly to all
mapped slave nodes. Slave nodes queue this data and consume it during
