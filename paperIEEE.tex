...
following we will present the related work published since then. An extensive
Programming and User Guide provides in-depth documentation on using and
programming Equalizer\cite{Eilemann:13}.
...
\begin{figure*}[ht]\center
  \subfigure[Time-Multiplex]{\label{fig:dplex}
    \includegraphics[width=0.24\textwidth]{images/dplex}}\hfil
  \subfigure[Tiles]{\label{fig:tile}
    \includegraphics[width=0.24\textwidth]{images/tile}}\hfil
  \subfigure[Pixel]{\label{fig:pixel}
    \includegraphics[width=0.24\textwidth]{images/pixel}}\hfil
  \subfigure[Subpixel]{\label{fig:subpixel}
    \includegraphics[width=0.24\textwidth]{images/subpixel}}\hfil
...
The monitor allows to reuse the rendering on another channel, typically for
monitoring a larger setup on a control workstation. Output frames on the display
channels are connected to input frames on a single monitoring channel. The
monitor equalizer changes the scaling factor and offset between the output and
input, so that the monitor channel has the same, but typically downscaled view,
as the originating segments.
...
which will synchronize the rendering of all contributing GPUs. The software
barrier executes a \textsf{glFinish} to ensure the GPU is ready to swap, a
Collage barrier (\sref{SEC_barrier}) to synchronize all segments, the swap
buffers call followed by a \textsf{glFlush} to ensure timely execution of the
swap command. The hardware swap barrier is implemented using the
